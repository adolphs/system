...
$(function() {
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                {type:'numbers'}
                ,{field:'dataName', title:'材料名称'}
                ,{field:'remarks', title:'填表须知'}
                ,{field:'type', title: '材料类型'}
                ,{field:'dataForm', title: '材料形式'}
        ...
        table.on('tool(userTable)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                //编辑
                // getUserAndRoles(data,data.dataId);
                window.location.href="/data/dataUpdate?dataId="+data.dataId;
...
function addUser(){
    // openUser(null,"新增材料");
    window.location.href="/data/dataAdd";
}
...
function getUserAndRoles(obj,id) {
    ...
    $("#dataForm").val(obj.dataForm);
    $("#templateName").val(obj.templateName);
    $("#templateUrl").val(obj.templateUrl);
    $("#blankName").val(obj.blankName);
    $("#blankUrl").val(obj.blankUrl);
    ...
    layui.use(['form', 'upload', 'layer'], function () {
        ...
        //检查项目添加到下拉框中
        if (obj.dataType == 1){
            tmp +='<option value="1" selected>通用材料</option><option value="2">情形材料</option>';
        }else{
            tmp +='<option value="2" selected>情形材料</option><option value="1">通用材料</option>';
        }
