...
$(function() {
    layui.use('table', function(){
        ...
        form.on('submit(userSubmit)', function(data){
            // TODO 校验
            formSubmit(data);
            // console.log(data,data.field.id);
            return false;
        });
        form.on('submit(userSubmit2)', function(data){
            ...
            // console.log($("#situationDetailsId2").val());
            // console.log($("#comboSituationId").val());
            $.ajax({
                url: '/combo/setDataAndComboSituation',
                data:{'dataId':$("#dataSelect").val(),'comboSituationDetailsId':$("#situationDetailsId2").val(),'comboSituationId':$("#comboSituationId").val()},
                type: 'post',
...
function submitAjax(obj){
    $.ajax({
        ...
        url: "/combo/setComboSituationDetails",
        success: function (data) {
            if (data == "ok") {
                layer.alert("操作成功",function(){
                    location.reload();
...
function submitAjax2(obj,currentUser){
    var comboDteId = $("#comboDteId").val();
    var comboId = $("#id").val();
    console.log(comboId);
    var situationDescribe = $("#situationDescribe").val();
    var situationDetailsDescribe = $("#situationDetailsDescribe").val();
    console.log(1)
    $.ajax({
        type: "POST",
        data: {"comboId":comboId,"id":comboDteId,"situationDescribe":situationDescribe,"detailsDescribe":situationDetailsDescribe},
        url: "/combo/addComboSituation",
        success: function (data) {
            if (data == "ok") {
                layer.alert("操作成功",function(){
                    location.reload();
