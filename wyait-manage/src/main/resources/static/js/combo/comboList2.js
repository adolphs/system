...
$(function() {
    layui.use('table', function(){
        ...
        tableIns=table.render({
            elem: '#uesrList'
            ,url:'/combo/getComboList?approvalType=2'
...
function recoverUser(obj) {
    if (confirm("是否确定通过"+ obj.comboName +"事项？")){
        $.ajax({
            url: '/combo/putApprovalType',
            data:{'comboId':obj.id,'approvalType':3},
            // dataType: 'json',
            type: 'post',
            success: function (data) {
                if (data == "ok"){
                    layer.alert("审批通过成功！",function(){
                        layer.closeAll();
                        //加载load方法
                        load(obj);//自定义
                    });
...
function put() {
    $.ajax({
        url: '/combo/putApprovalType',
        data:{'comboId':$("#comboId").val(),'approvalType':4,'approvalText':$("#approval_type").val()},
        // dataType: 'json',
        type: 'post',
        success: function (data) {
            if (data == "ok"){
